...
var bmw = carMaker({ name: "BMW" }); // Functional Pattern // Pros: simple, relies on basic closure // Cons: all methods are instantiated anew for every instance, i.e. bloat // The most basic type of class, aka. the module pattern
...
var bmw = carMaker("BMW"); // Functional-Shared Pattern // Pros: less memory usage than Functional // Cons: If a method is changed in `methods`, old instance aren't updated // Constructors extend the created object
...
// The new instance is created by the constructor
var bmw = carMaker("BMW"); // Prototypal Pattern // Pros: uses prototype chain instead of reference, // prototype lookup is dynamic so updates in prototype reflect in all instances // Cons: excess code, not as declarative/descriptive
...
// The new instance is created by the constructor
var bmw = carMaker("BMW"); // Pseudoclassical Pattern // Pros: declarative code, dynamic property lookup // Cons: enforces a class hierarchy which can be too rigid for large projects // (this applies to inheritance-based code in general)
...
var bmw = new Car("BMW"); // Pseudoclassical Pattern (ES6) // Purely syntactic sugar to make the pseudoclasssical style less "pseudo" // `class` declares the prototype object // and can be paired with `extends` for inheritance
